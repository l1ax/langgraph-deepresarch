...
export class Executor {
  ...
  /** 处理接收到的 chunk 数据 */
  @action.bound
  private handleChunk(chunk: Executor.StreamChunk) {
    ...
    if (chunk.event === 'custom' && chunk.data) {
      ...
      if (wrappedData.eventType) {
        this.addEvent({
          id: chunk.id, // 使用流式数据自带的 id
   ...
   */
  @action.bound
  async invoke(content: string): Promise<void> {
    ...
    try {
      ...
      for await (const chunk of stream) {
        ...
        this.handleChunk(chunk as Executor.StreamChunk);
      }
    } finally {
...
export namespace Executor {
  ...
  /** 输出事件接口（前端存储的格式，添加了 id） */
  export interface OutputEvent<T = unknown> {
    /** 事件唯一标识（来自流式数据的 run_id） */
    id: string;
    /** 事件类型 */
    eventType: EventType;
    /** 事件内容 */
